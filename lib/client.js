...
function Client(options) {
    ...
    console.log('XXXXXXXXXXXXXXXXXXXXXXX function Client(options) { ' + JSON.stringify(options));
    console.log('XXXXXXXXXXXXXXXXXXXXXXX XXXXXXXXXXXXXXXXXXXXXXX ');

    self.options = _.defaultsDeep(options || {}, {
        clientId: 'no-kafka-client',
        connectionString: options.connectionString || process.env.KAFKA_URL || 'kafka://127.0.0.1:9092',
        ssl: options.ssl || {},
...
Client.prototype.init = function () {
    var self = this;
    console.log('XXXXXXXXXXXXXXXXXXXXXXX XXXXXXXXXXXXXXXXXXXXXXX ');
    console.log('XXXXXXXXXXXXXXXXXXXXXXX Client.prototype.init = function () { ' + JSON.stringify(self.options));
    console.log('XXXXXXXXXXXXXXXXXXXXXXX XXXXXXXXXXXXXXXXXXXXXXX ');
    self.initialBrokers = self.options.connectionString.split(',').map(function (hostStr) {
        ...
        if (!/^([a-z]+:)?\/\//.test(hostStr)) {
            if(_.isEmpty(self.options.ssl)){
                self.socket.connect(self.port, self.host, self.ssl, function () {
                    self.connected = true;
                    resolve();
                });
            }
